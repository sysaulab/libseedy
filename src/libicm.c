...
{
/* read before modifying https://sysau.com/Derivatives.pdf */
...
void __icm_pause(icm_state_t* icm){
    for(int x = 0; x < _ICM_NUM_THREADS; x++){
...
{
    for( int i = 0; i < _ICM_NUM_THREADS; i++ )
    {
        state->nodes[i] = 0;
        state->threads[i].source = &(state->nodes[i]);
        state->threads[i].sink = &(state->nodes[(i + 1) % _ICM_NUM_THREADS]);
...
void icm_fill64(icm_state_t* state, uint64_t* buffer, uint64_t count)
{
    ...
    {
        usleep(_ICM_WAIT);
        for(int y = 0; y < _ICM_NUM_THREADS; y++ )
        {
