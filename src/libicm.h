...
#define _ICM_WAIT           30    // Sleeping time in microseconds between picks
#define _ICM_NUM_THREADS    3     // number of threads and nodes in ICM (3 recommended)
...
typedef struct
{
    volatile uint64_t nodes[_ICM_NUM_THREADS];
    icm_thread_t threads[_ICM_NUM_THREADS];
}
icm_state_t;
